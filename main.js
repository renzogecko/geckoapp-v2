...
// Sticky láser eliminado — el guardado se hace desde el botón al pie del tab
                        ...
                        <div id="bastidorRevestimientoExtra" class="hidden animate-in fade-in slide-in-from-top-2">
                             ...
                             <select id="bastidorRevestimientoMaterial" onchange="window.calcularCostoBastidores()" class="${inputStyle} gecko-select py-3 bg-[#131314] cursor-pointer appearance-none">
                                ${(window.materiales || []).filter(m => {
                                    ...
                                    return (m.categoria === 'vinilos_lonas' && nombreLower.includes('lona'))
                                        || m.categoria === 'chapas'
                                        || nombreLower.includes('acm');
        ...
        if (currentCat === 'laser_cnc' || currentCat === '3d') {
            ...
            if (item && item.costo > 0) {
                ...
                if (currentCat === '3d') {
                    ...
                    // Recalcular para que el auditor de Impresión 3D muestre estado vacío
                    if (typeof window.calcularCosto3D === 'function') {
                        setTimeout(() => window.calcularCosto3D(), 50);
                    }
                } else if (currentCat === 'laser_cnc') {
                    // Recalcular el auditor de Láser/CNC (NO el de Impresión 3D)
                    if (typeof window.calcularCostoCorte === 'function') {
                        setTimeout(() => window.calcularCostoCorte(), 50);
                    }
                }
...
function abrirFichaCliente(nombre) {
    ...
    document.getElementById('tbodyFichaActivos').innerHTML = activos.length === 0
        ? '<tr><td colspan="6" class="py-8 text-center text-[#71717a] text-sm italic">Sin trabajos activos</td></tr>'
        : activos.map(p => `
            <tr class="border-b border-[#333333] last:border-0 hover:bg-[#131314] transition-colors">
                ...
                <td class="py-3 px-4 text-right font-black text-[11px] text-[#a1a1aa]">
                    $${Math.round(p.total || 0).toLocaleString('es-AR')}
                </td>

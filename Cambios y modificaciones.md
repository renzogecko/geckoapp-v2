...
- ✅ Gastos Fijos — campo `categoria` no se guardaba en MySQL: agregada columna `categoria` a tabla `gastos_fijos` y actualizado `api.php` con INSERT y UPDATE que incluyen el campo.
- ✅ Gastos Fijos — race condition al cargar varios gastos seguidos: eliminada actualización prematura del `_cache` en `_sincronizarArray` de `gecko-api.js`. El cache ahora se actualiza solo al finalizar todos los POSTs.
...
- ✅ Bug crítico #1: Imágenes adjuntas y metadata no persistían tras recargar. Agregada columna metadata LONGTEXT en tabla presupuestos.
